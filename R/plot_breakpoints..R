...
plotBreakpoints<-function(domainDataFrame=domainDataFrame,exons=exons,geneposition=geneposition){
  ...
  if (geneposition=="Left"){
    ...
    p1<-ggplot(geneExons,aes(y=geneExons$start,x=geneExons$transcript)) + geom_hline(yintercept=as.numeric(domainDataFrame$LeftBreakpoint),linetype="dashed",color="violet") + geom_linerange(ymin=geneExons$start,ymax=geneExons$end,size=3)+geom_linerange(ymin=geneExons$start,ymax=geneExons$end,size=6,aes(col=geneExons$DESC,x= gene),color="grey") + coord_flip() +theme_Publication()+xlab("Transcript")+ylab("Coordinate")+ggtitle(paste0(geneposition," fused Gene:",gene))+labs(fill="Domain")
  ...
  if(geneposition=="Right"){
    ...
    geneExons$transcript<-factor(geneExons$transcript,levels=uniqtranscript[length(uniqtranscript):1])
    # basic plot for exon and gene
    p1<-ggplot(geneExons,aes(y=geneExons$end,x=geneExons$transcript)) + geom_hline(yintercept=as.numeric(domainDataFrame$RightBreakpoint),linetype="dashed",color="violet") + geom_linerange(ymin=geneExons$start,ymax=geneExons$end,size=3)+geom_linerange(ymin=geneExons$start,ymax=geneExons$end,size=6,aes(col=geneExons$DESC,x= gene),color="grey") + coord_flip() +theme_Publication()+xlab("Transcript")+ylab("Coordinate")+ggtitle(paste0(geneposition," fused Gene:",gene))
  }
