...
p1<-ggplot(fusion_chrom,aes(x=!!as.name(groupby),fill=fusion_chrom$Distance,alpha=0.75))+geom_bar()+theme_Publication()+theme(legend.position = "top")+xlab(as.name(groupby))+ylab("Count")+guides(alpha=FALSE)+theme(axis.text.x  = element_text(angle = 45))+labs(fill = "Distance")
...
p4<-ggplot(kinase_fusion,aes(x=Domain,y=Type.ct,fill=gene_position,alpha=0.95))+geom_col()+theme_Publication()+ylab( "Count")+xlab("Kinase domain retained")+guides(alpha=FALSE)+theme(axis.text.x  = element_text(angle = 45,hjust = 1,size=12),axis.text.y = element_text(angle=0,vjust =2,size=12))+labs(fill = "Gene Position")
} else {
