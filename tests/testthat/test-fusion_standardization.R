...
customtypefile <- read.delim(system.file("extdata", "custom_type_example.tsv", package = "annoFuseData"), check.names = FALSE)
...
test_that("Standardizing Custom type calls with config file", {
  expect_equal(colnames(CustomFusioncalls), c("Sample", "FusionName", "Gene1A", "Gene1B", "Gene2A", "Gene2B", "Fusion_Type", "annots"))
