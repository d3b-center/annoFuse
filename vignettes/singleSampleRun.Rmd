...
In this vignette we demonstrates using annoFuse, to filter out fusions known to be artifactual or with low read support and retain high-quality fusion calls from STAR-Fusion and Arriba for a single sample.
We also annotate these high quality fusion calls and capture known and putative oncogenic driver fusions previously reported in TCGA or fusions containing gene partners that are known oncogenes, tumor suppressor genes, COSMIC genes, or transcription factors.
 ...
 * We used [FusionAnnotator](https://github.com/FusionAnnotator/FusionAnnotator/tree/v0.2.0) to annotate arriba files to specifically identify “red flag” fusions found in healthy tissues or in gene homology databases saved as column "annots" to match annotation in STAR-Fusion calls. 
 Users can provide any annotation they seem useful for their dataset to this field and filter using the param `artifactFilter` in `annoFuse_single_sample()`
